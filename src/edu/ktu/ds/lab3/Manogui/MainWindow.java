package edu.ktu.ds.lab3.Manogui;

import edu.ktu.ds.lab3.Staskevicius.Device;
import edu.ktu.ds.lab3.Staskevicius.DeviceGenerator;
import edu.ktu.ds.lab3.Staskevicius.HashMapOa;
import edu.ktu.ds.lab3.Staskevicius.ParsableHashMapOa;
import edu.ktu.ds.lab3.Staskevicius.SimpleBenchmark;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Device[] devicesArray = devicesGenerator.generateShuffleDevicesAndIds(sizeOfGenSet, sizeOfInitialSubSet);
            for (Device d : devicesArray) {
                map.put(
                        devicesGenerator.getDeviceId(), //raktas
                        d);
            }
    ...
    private void mapPut() {
        Device device = devicesGenerator.getDevice();
        map.put(
                devicesGenerator.getDeviceId(), // Raktas
                device);
        table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
        ...
        KsGui.oun(taEvents, device, MESSAGES.getString("mapPut"));
    }
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                map = new ParsableHashMap<>(String::new, Device::new, initialCapacity, loadFactor, ht);
                break;
                
            case 3:
                map = new ParsableHashMapOa<>(String::new, Device::new, initialCapacity, loadFactor, ht);
                break;
            // ...
